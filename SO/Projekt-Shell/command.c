...
noreturn void external_command(char **argv) {
  ...
  if (!index(argv[0], '/') && path) {
    /* TODO: For all paths in PATH construct an absolute path and execve it. */
      while(true){
        size_t off = strcspn(path, ":"); //number of bytes you want to read until u get ":"
      
        if(!off) break;
      
        char *full_path = strndup(path, off); //read off bytes from path (allocates memory)
        ...
        path = path + off + 1; //increment pointer to path by num of bytes u just read and + 1 cause of ":"
    }
  } else {
