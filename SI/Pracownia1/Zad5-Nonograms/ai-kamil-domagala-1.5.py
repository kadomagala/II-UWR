...
def solve(rows, cols):
    ...
    MAX_NUM_OF_CHANGES = 1000
    num_of_changes = 0
    small_probability = 0.1
...
def sum_level(row, col, rows, cols, nonogram):
    ...
    """
    current_row_dist = opt_dist(nonogram[row, :], rows[row])
    current_col_dist = opt_dist(nonogram[:, col], cols[col])
    current_sum = current_row_dist + current_col_dist

    nonogram[row][col] = '1' if nonogram[row][col] == '0' else '0'

    changed_row_dist = opt_dist(nonogram[row, :], rows[row])
    changed_col_dist = opt_dist(nonogram[:, col], cols[col])
...
def get_unsolved_rows_idxs(rows, nonogram):
    ...
    for i in range(len(rows)):
        if opt_dist(nonogram[i, :], rows[i]) > 0:
            to_pick.append(i)
    return to_pick
...
def get_unsolved_cols_idxs(cols, nonogram):
    ...
    for i in range(len(cols)):
        if opt_dist(nonogram[:, i], cols[i]) > 0:
            to_pick.append(i)
    return to_pick
...
if __name__ == '__main__':
    if DEBUG:
        result = solve([7, 7, 7, 7, 7, 7, 7], [7, 7, 7, 7, 7, 7, 7])
        print_nonogram(result)
    else:
        with open('zad5_input.txt', 'r') as r:
            with open('zad5_output.txt', 'w') as w:
                while True:
                    line = r.readline()
                    if not line:
                        break
                    ...
                    for x in range(rows_num):
                        c = r.readline()
                        rows_ls.append(int(c))
                    for y in range(cols_num):
                        c = r.readline()
                        cols_ls.append(int(c))
